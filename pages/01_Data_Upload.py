...
uploaded_file = st.file_uploader(
    ...
    type=['csv'],
    help=
    "Upload a CSV file containing customer data with features and churn labels"
)
...
if uploaded_file is not None:
    try:
        ...
        is_transaction_data = all(col in data.columns
                                  for col in transaction_columns)

        if is_transaction_data:
            st.info(
                "📊 Detected transaction data. Converting to customer-level churn dataset..."
            )
            ...
            with col1:
                churn_days = st.number_input(
                    ...
                    value=30,
                    help=
                    "Customers with no activity for this many days are considered churned"
                )
            with col2:
            ...
            if st.button("🔄 Convert Transaction Data", type="primary"):
                with st.spinner(
                        "Converting transaction data to churn dataset..."):
        ...
        with col2:
            ...
            missing_df = pd.DataFrame({
                ...
                'Missing %':
                (data.isnull().sum() / len(data) * 100).round(2).values
            })
        ...
        st.write(
            "Select the column that represents customer churn (must have exactly 2 unique values)"
        )
        ...
        for col in data.columns:
            ...
            if unique_count == 2:
                ...
                binary_columns.append(
                    f"{col} (values: {', '.join(map(str, values))})")
                binary_column_names.append(col)

        if binary_columns:
            st.success(
                f"✅ Found {len(binary_columns)} binary columns suitable for churn prediction:"
            )
            ...
            default_target = None
            churn_keywords = [
                'churn', 'churned', 'left', 'exited', 'attrition'
            ]
            ...
            if default_target and default_target in binary_column_names:
                st.info(
                    f"💡 Automatically detected '{default_target}' as likely churn column"
                )
                default_index = binary_column_names.index(default_target)
            else:
                default_index = 0 if binary_column_names else None

        else:
            st.warning(
                "⚠️ No binary columns found. Churn prediction requires a column with exactly 2 unique values."
            )
            default_index = 0

        if binary_column_names and default_index is not None:
            target_column = st.selectbox(
                "Select target column:",
                options=binary_column_names,
                index=default_index,
                help=
                "Choose the column that indicates whether a customer churned (must have exactly 2 values)"
            )
        else:
            st.warning("⚠️ No valid binary columns available for selection.")
            target_column = None
        ...
        if st.button("Process Data", type="primary"):
            with st.spinner("Processing data..."):
                ...
                processed_data, feature_columns = processor.preprocess_data(
                    data, target_column)
    ...
    st.write(
        "Your CSV should contain customer features and a binary target column indicating churn. Example:"
    )
    ...
    sample_data = pd.DataFrame({
        ...
        'payment_method':
        ['Credit card', 'Bank transfer', 'Electronic check', 'Credit card'],
        'churn': [1, 0, 0, 1]  # Binary: 1 = churned, 0 = stayed
    })

    st.write("**Key requirements:**")
    st.write(
        "• Target column must have exactly 2 unique values (e.g., 0/1, Yes/No, True/False)"
    )
    st.write(
        "• Include customer features like demographics, usage, and service information"
    )
    st.write(
        "• Remove any customer identifiers before analysis (they'll be ignored during preprocessing)"
    )
    ...
    if st.button("📥 Download Sample Dataset",
        ...
        sample_dataset = pd.DataFrame({
            ...
            'tenure_months':
            np.random.randint(1, 72, n_samples),
            'monthly_charges':
            np.round(np.random.uniform(20, 120, n_samples), 2),
            'total_charges':
            np.round(np.random.uniform(50, 8000, n_samples), 2),
            'contract_type':
            ...
            'internet_service':
            np.random.choice(['DSL', 'Fiber optic', 'No'], n_samples),
            'online_security':
        ...
        churn_prob = (
            0.1 +  # base rate
            0.3 * (sample_dataset['contract_type'] == 'Month-to-month') + 0.2 *
            (sample_dataset['tenure_months'] < 12) + 0.1 *
            (sample_dataset['monthly_charges'] > 80) + 0.1 *
            (sample_dataset['payment_method'] == 'Electronic check') + 0.1 *
            (sample_dataset['senior_citizen'] == 1))
        sample_dataset['churn'] = np.random.binomial(
            1, np.minimum(churn_prob, 0.8), n_samples)
        ...
        st.download_button(
            ...
            mime="text/csv",
            help=
            "This sample dataset contains 1000 customers with realistic churn patterns"
        )
